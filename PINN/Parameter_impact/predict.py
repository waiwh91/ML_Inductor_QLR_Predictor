...
model = trainer.PINN()
model.load_state_dict(torch.load('/home/martin/ML_Inductor_QLR_Predictor/PINN/saved_models/PINN_model.pth'))
...
x_train_50 = torch.from_numpy(data_50[:, :6]).int()
f_train_50 = torch.from_numpy(data_50[:, 6]).float()

x_train_100 = torch.from_numpy(data_100[:, :6]).int()
...
r_pre_50, l_pre_50 = model(torch.log(x_train_50), torch.log(f_train_50)).T.detach().numpy()
...
r_pre_100, l_pre_100 = model(torch.log(x_train_100), torch.log(f_train_100)).T.detach().numpy()
