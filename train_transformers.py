...
def train_transformers():
    ...
    pre_train_dataloader = models.dataloader.dataloader(x_pretrain, y_pretrain, 1024)
    ...
    x_train, y_train, x_test, y_test = torch.from_numpy(spliter.split_data(data, 1.0)).float().to(device)
    dataloader = models.dataloader.dataloader(x_train,y_train,32)
